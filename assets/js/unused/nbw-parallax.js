...
$(document).ready(function() { //when the document is ready...
	...
	var $window = $(window);
	var $firstBG = $('#history');
	var $firstSprite = $('#historySprite');
	var $secondBG = $('#home');
	var $thirdBG = $('#present');
	var $fourthBG = $('#future');
	var trainers = $("#present .bg");
	
	var windowHeight = $window.height(); //get the height of the window
	
	
	//apply the class "inview" to a section that is in the viewport
	$('#history, #home, #present, #future').bind('inview', function (event, visible) {
			if (visible == true) {
	...
	*/
	...
	function spritePos(x, windowHeight, pos, adjuster, inertia){
	...
	function Move(){ 
		...
		if($firstBG.hasClass("inview")){
			...
			$firstBG.css({'backgroundPosition': newPos(50, windowHeight, pos, 100, 0.2)});
